...
function Profile() {
  ...
  useEffect(() => {
    if (contractUserNFTS) {
      getUserNfts(account, contractUserNFTS)
    }
  }, [])

  const getUserNfts = async (account, contractUserNFTS) => {
    const res = await contractUserNFTS.getUserNFTs(account)
    console.log({ res })
    const data = []
    for (let i = 0; i < res.length; i++) {
      const index = res[i].toString()
      const image = await contractUserNFTS.tokenURI(index)
      data.unshift(image)
    }
    setMyNfts(data)
  }
    ...
    <VStack pt="9rem" pb="9rem">
      ...
      <div className={styles.profileItem}>
        <p className={styles.profileParaghap}>
          Total NFTs owned: {myNfts.length}
        </p>
        <p className={styles.profileParaghap}>
          {account}
      ...
      <section id="task">
        ...
        <SimpleGrid columns={{ base: 4, md: 4, lg: 5 }} gap={5} pt={10}>
          {myNfts?.map((url, idx) => (
            <MyNfts url={url} key={idx} handleClick={handleClick} />
          ))}
        </SimpleGrid>
